    ...
    PausableUpgradeable
{
    function initialize(
        ...
        address chainLinkMaticToEth
    ) public virtual initializer {
        __WhyNotWorldGameNFT_init(
            name,
            symbol,
            baseTokenURI,
            chainLinkMaticToEth
    ...
    struct Token {
        ...
        uint8 discountedAmountNumerator;
    }
    ...
    function __WhyNotWorldGameNFT_init(
        ...
        __Pausable_init();
        __WhyNotWorldGameNFT_init_unchained(
            name,
            symbol,
            baseTokenURI,
            chainLinkMaticToEth
    ...
    function __WhyNotWorldGameNFT_init_unchained(
        ...
        priceFeed = AggregatorV3Interface(chainLinkMaticToEth);
    ...
    function safeMintAlgo(
        ...
        Algo buyAlgo_,
        uint8 discountedAmountNumerator_
        ...
        uint256 diffToReturn = validateAmount(scenes_, msg.value);
        ...
        if (scenes_[0] < getMaxSceneWhichCanBeBought(buyAlgo_)) {
            ...
            //Check if there is already a token for next Scene which is sold but not reserved.
            ...
            if (
                ...
                payable(to_).transfer(amountToRefund(reserveScene));
            } else {
        ...
        if (
            scenes_[0] > Scene.Scene1 &&
            currentToken.discountedAmountNumerator == 0
        ) {
            ...
            if (tokenIndexToRefund[previousScene].length > 0) {
                ...
                if (
                    ...
                    payable(
                        ownerOf(sceneTokens[previousScene][refundIndex].tokenId)
                    ).transfer(amountToRefund(scenes_[0]));
                } else {
        ...
        if (diffToReturn > 0) {
            payable(msg.sender).transfer(diffToReturn);
        }
    }
    ...
    function isReservedDiscountedTokenForScene(Scene scene)
        private
        returns (uint8)
    {
        uint256 currentTokenIndex = sceneTokenCount[scene];
        ...
        return currentToken.discountedAmountNumerator;
    }
    ...
    function validateAmount(Scene[] memory scenes_, uint256 priceInMatic)
        private
        returns (uint256)
    {
        uint256 maticValueInUSD = uint256(getLatestPrice());
        uint256 priceInUSD = (maticValueInUSD * priceInMatic) / (10**18);

        uint256 sellPriceInUSD;

        uint8 discountedPrice = isReservedDiscountedTokenForScene(scenes_[0]);
        if (discountedPrice > 0) {
            sellPriceInUSD =
                ((120 * (3**(uint256(scenes_[0]) - 1))) * discountedPrice) /
                3;
        } else {
            sellPriceInUSD = 120 * (3**(uint256(scenes_[0]) - 1));
        }

        for (uint256 i = 1; i < scenes_.length; i++) {
            sellPriceInUSD += ((120 * (3**(uint256(scenes_[i]) - 1))) / 3);
        }
        uint256 sellPrice = sellPriceInUSD * (10**8);
        require(sellPrice > 0, "No Price defined for Algo and Scene");

        require(priceInUSD >= sellPrice, "Wrong Price Sent to buy nft");

        if (priceInUSD > sellPrice) {
            uint256 sellPriceInMatic = (sellPrice * (10**18)) / maticValueInUSD;

            return (priceInMatic - sellPriceInMatic);
        }

        return 0;
    }

    function amountToRefund(Scene scene) private returns (uint256) {
        uint256 maticValueInUSD = uint256(getLatestPrice());

        uint256 sellPrice = (120 * (3**(uint256(scene) - 1))) * (10**8);

        uint256 sellPriceInMatic = (sellPrice * (10**18)) / maticValueInUSD;

        return (sellPriceInMatic * 2) / 3;
    }
	...
	function amountToPay( Scene[] memory scenes_) external view returns(uint256)
    {
        uint256 maticValueInUSD=uint256(getLatestPrice());
     
        uint256 sellPriceInUSD = 120 * (3**(uint256(scenes_[0]) - 1));
